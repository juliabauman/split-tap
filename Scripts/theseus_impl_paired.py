...
##Create single col for spacer pair
def make_guide_id(df):
    required = ['SPACER1', 'SPACER2']
    for c in required:
        if c not in df.columns:
            raise ValueError(f"Missing required column: {c}")
    df["spacer1_spacer2"] = df["SPACER1"] + df["SPACER2"]
    return df
...
def calculate_tpt(umi_obs):
  ...
  for c in ['cell', 'umi', 'spacer1_spacer2']:
        if c not in umi_obs.columns:
            raise ValueError(f"Missing required column: {c}")

  counts = (umi_obs.groupby(['cell','umi','spacer1_spacer2'], as_index=False).size()
...
print("Generating spacer combo guide_id...")
umi_obs = make_guide_id(umi_obs)
