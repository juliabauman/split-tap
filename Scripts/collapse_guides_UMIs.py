...
def collapse_by_hamming(sequences, counts, max_dist=1):
    """
    Collapse similar sequences within max_dist using greedy strategy.
    """
    seq_counts = Counter()
...
for group_key, group_df in umi_df.groupby(group_cols):
    ...
    n_corr1 = len(set(spacer1s)) - len(set(collapse_map1.values()))
    n_corr2 = len(set(spacer2s)) - len(set(collapse_map2.values()))
    spacer_corrections.append({
        'group': group_key,
        'n_spacer1_corrected': n_corr1,
        'n_spacer2_corrected': n_corr2
    })
...
final_df = final_df.rename(columns={
    'collapsed_spacer1': 'SPACER1',
    'collapsed_spacer2': 'SPACER2'
})
...
# ── Correction logs ───────────────────────────────────────────────────────────
# UMI corrections
corrections_df = pd.DataFrame(umi_corrections, columns=['group', 'n_before', 'n_after', 'n_corrected'])
corrections_df[['Cell_Barcode', 'SPACER1', 'SPACER2']] = pd.DataFrame(corrections_df['group'].tolist(), index=corrections_df.index)
corrections_df.drop(columns='group', inplace=True)
corrections_df.to_csv(umi_corrections_file, sep='\t', index=False)

# Spacer corrections
spacer_corr_df = pd.DataFrame(spacer_corrections)
spacer_corr_df[['Cell_Barcode', 'Molecular_Barcode']] = pd.DataFrame(spacer_corr_df['group'].tolist(), index=spacer_corr_df.index)
spacer_corr_df.drop(columns='group', inplace=True)
spacer_corr_df.to_csv(spacer_corrections_file, sep='\t', index=False)
