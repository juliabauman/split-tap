...
def extract_info(fastq_file, output_txt, summary_txt):
    ...
    with gzip.open(fastq_file, "rt") as fq, open(output_txt, "w") as out_txt, open(summary_txt, "w") as summary_out:
        ...
        out_txt.write("cell\tumi\tSPACER1\tSPACER2\ttRNA\tspacer1_spacer2\n")
        ...
        while True:
            ...
            spacer1_spacer2 = spacer1 + spacer2
            ...
            out_txt.write(f"{cell_barcode}\t{umi}\t{spacer1}\t{spacer2}\t{trna}\t{spacer1_spacer2}\n")
...
def add_num_obs_column(infile, outfile_with_counts):
    """
    Reads input file, counts occurrences of each unique (cell, umi, SPACER1, SPACER2, tRNA) combo,
    and saves a new file with a Num_Obs column.
    """
    df = pd.read_csv(infile, sep="\t")
    df_counts = df.groupby(['cell', 'umi', 'SPACER1', 'SPACER2', 'tRNA', 'spacer1_spacer2']).size().reset_index(name='Num_Obs')
