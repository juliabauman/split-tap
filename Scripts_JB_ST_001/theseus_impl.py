...
def calculate_tpt(umi_obs):
  ...
  ## Need to first count the reads of cell-umi-gRNA triplets:
  umi_obs = umi_obs.groupby(['cell', 'umi', 'gRNA_name']).size().reset_index(name='Num_Obs')

  # calculate the total number of reads per cell-umi barcode combination
  reads = umi_obs.groupby(['cell', 'umi'])['Num_Obs'].sum()
  ...
  umi_obs = pd.merge(umi_obs, reads, how = 'left', on = ['cell', 'umi'])
